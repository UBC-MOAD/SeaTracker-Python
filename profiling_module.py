# Copyright 2017-2018 Susan Allen, Doug Latornell, and The University of
# British Columbia
...
def main():
    t_mask, e1u, e2v, e3w0, totaldepth = seatracker_module.initialize_mesh(
        mesh_mask_path
    )
    (
        u, v, w, tcorrs, t_coords, u_coords, v_coords, w_coords, deltat,
        nextindex, e3w, udataset, vdataset, wdataset, tdataset
    ) = seatracker_module.get_initial_data(
        u_field_path, v_field_path, w_field_path, tracer_fields_path,
        totaldepth, e3w0
    )
    tc, yi = seatracker_module.grid_points(t_coords, t_mask, 25 - 1, 446 - 1, 304 - 1)
    t0 = tcorrs[0]
    dt = deltat / 10
    y0 = numpy.copy(yi)
    yp = numpy.ndarray.flatten(y0)
    integrator = ode(seatracker_module.derivatives).set_integrator(
        'dopri5', atol=0.01
    )
    ...
    for count in range(2):
        # update arrays
        tcorrs, u_coords, v_coords, w_coords, u, v, w, nextindex, e3w = seatracker_module.update_arrays(
